...
try:
    ...
    exit(1)
print(str(train_datas.data[0].data.shape))#
...
W1 = tf.Variable(tf.random_normal([2500, 700],initializer=tf.contrib.layers.xavier_initializer()))
b1 = tf.Variable(tf.random_normal([700]))
W2 = tf.Variable(tf.random_normal([700, 500],initializer=tf.contrib.layers.xavier_initializer()))
b2 = tf.Variable(tf.random_normal([500]))
W3 = tf.Variable(tf.random_normal([500, 256],initializer=tf.contrib.layers.xavier_initializer()))
b3 = tf.Variable(tf.random_normal([256]))
W4 = tf.Variable(tf.random_normal([256, nb_classes],initializer=tf.contrib.layers.xavier_initializer()))
...
with tf.Session() as sess:
    ...
    # get predict value
    pred = sess.run(tf.argmax(hypothesis, 1), feed_dict={X: one_data[...,:-16]})
